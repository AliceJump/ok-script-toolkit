...
- 支持侧边栏（包含模板面板、模板素材和临时截图三个视图）和编辑器大窗口两种浏览方式。

#### 临时截图

- **ok-script 模板: 临时截图** 视图是一个最多 10 张的暂存区（超出自动淘汰最早一张），用于快速取素材：
...
- **标注编辑器**同样支持该模式：工具栏「坐标 (C)」或按 `C` 键后框选，直接复制同样的
  `x,y,tox,toy` 归一化坐标，不会创建标注框、不改动 COCO。快捷键可用
...
- 侧边栏分为 **ok-script 工具**（工具箱 + 任务启动）和 **ok-script 模板**（模板面板 + 模板素材 + 临时截图）两个独立容器。
