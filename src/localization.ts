...
const ZH_CN: WebviewStrings = {
  ...
  accountStoreUnavailable: '无法读取多账户存储（该项目缺少 account_scope_store 模块，或 Python 环境不可用）。',
...
const EN: WebviewStrings = {
  ...
  accountStoreUnavailable: 'Unable to read the multi-account store (project lacks an account_scope_store module, or the Python env is unavailable).',
...
const ZH_TW: WebviewStrings = {
  ...
  accountStoreUnavailable: '無法讀取多帳號儲存（專案缺少 account_scope_store 模組，或 Python 環境不可用）。',
...
const JA: WebviewStrings = {
  ...
  accountStoreUnavailable: 'マルチアカウントストアを読み取れません（account_scope_store モジュール欠落、または Python 環境が利用不可）。',
...
const KO: WebviewStrings = {
  ...
  accountStoreUnavailable: '멀티 계정 스토어를 읽을 수 없습니다(account_scope_store 모듈 누락 또는 Python 환경 사용 불가).',
...
const ES: WebviewStrings = {
  ...
  accountStoreUnavailable: 'No se pudo leer el almacén multi-cuenta (falta el módulo account_scope_store o el entorno Python no está disponible).',
